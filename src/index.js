...
module.exports = class Reader extends Component {
  ...
  initiate(props = this.props) {
    ...
    for (var deviceIdOffset = 0; deviceIdOffset > 6; deviceIdOffset++) {
      //var vConstraintsPromise = supported.facingMode || isFirefox ? Promise.resolve(props.constraints || constraints) : getDeviceId(facingMode, i).then(function (deviceId) {
      var vConstraintsPromise = getDeviceId(facingMode, deviceIdOffset).then(
        function (deviceId) {
